...
bool ValidateBarrierLayoutToImageUsage(layer_data *device_data, const VkImageMemoryBarrier *img_barrier, bool new_not_old,
    ...
    if (msg_code != VALIDATION_ERROR_UNDEFINED) {
        skip |= log_msg(report_data, VK_DEBUG_REPORT_ERROR_BIT_EXT, VK_DEBUG_REPORT_OBJECT_TYPE_UNKNOWN_EXT, 0, __LINE__, msg_code, "DS", "%s: Image barrier 0x%p %sLayout=%s is not compatible with image 0x%p usage flags 0x%x. %s", func_name, img_barrier, ((new_not_old) ? "new" : "old"), string_VkImageLayout(layout), img_barrier->image, usage_flags, validation_error_map[msg_code]);
    }
    return skip;
}
